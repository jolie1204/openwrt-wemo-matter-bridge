...
static we_device_ctx_t *we_get_device_ctx(int wemo_id, int create)
{
    ...
    // Initialize only the non-mutex/cond fields to avoid corrupting threading primitives
    // If this slot was never used, mutex/cond are already zero-initialized (static array)
    // If it was used before, we must not destroy mutex/cond while holding table lock
    // as other code might still reference them. Just reinitialize if needed.
    if (free_slot->in_use == 0 && free_slot->wemo_id == 0) {
        // Fresh slot - initialize mutex and cond
        pthread_mutex_init(&free_slot->lock, NULL);
        pthread_cond_init(&free_slot->cond, NULL);
    }

    // Clear only the data fields, not the mutex/cond
    free_slot->wemo_id = wemo_id;
    free_slot->in_use = 1;
    free_slot->last_state_valid = 0;
    free_slot->state_seq = 0;
    free_slot->pending_response_ready = 0;
    free_slot->pending_completion_ready = 0;
    free_slot->last_response_valid = 0;
    free_slot->last_completion_valid = 0;
    memset(&free_slot->last_state, 0, sizeof(free_slot->last_state));
...
static int we_fetch_cached_device_list_once(struct we_device_list *out)
{
    ...
    req_proto.client_id = we_client_id ? we_client_id : we_make_client_id();
    req_proto.request_id = req_id;
    req_proto.op = CMD_LIST_DEVICES;
    req_proto.wemo_id = 0;
    req_proto.status = WE_STATUS_OK;
    req_proto.payload_len = 0;

    memset(&ipchdr, 0, sizeof(ipchdr));
    ipchdr.wemo_id = 0;
    ipchdr.cmd = CMD_PROTO;
    ipchdr.size = (int)sizeof(req_proto);
    ...
    if (rsp_proto->payload_len < sizeof(struct we_device_list)) {
        return WE_STATUS_INVALID;
    }
    memcpy(out, rsp_buf + sizeof(struct we_proto_hdr), sizeof(*out));
    if (out->count < 0) {
        out->count = 0;
    }
    if (out->count > WE_DEVICE_LIST_MAX_ITEMS) {
...
int we_init()
{
    ...
    // Initialize all device context mutexes and condition variables
    for (int i = 0; i < WE_MAX_TRACKED_DEVICES; i++) {
        if (we_device_table[i].in_use == 0 && we_device_table[i].wemo_id == 0) {
            pthread_mutex_init(&we_device_table[i].lock, NULL);
            pthread_cond_init(&we_device_table[i].cond, NULL);
        }
    }
    ...
    // Set a larger stack size (8MB) to prevent stack overflow
    pthread_attr_setstacksize(&attr, 8 * 1024 * 1024);

    if (pthread_create(&we_ipc_thread, &attr, we_comm_task, NULL) != 0) {
