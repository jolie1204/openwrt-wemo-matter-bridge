...
static const char *ipc_cmd_to_string(int cmd)
{
    ...
    case CMD_LIST_DEVICES: return "list_devices";
...
static int listen_socket(void)
{
    ...
    LOG_INFO_MSG("ipc listen on %s:%d", ipc_bind_addr, ipc_port);
...
static int wemo_ctrl_list_devices(struct we_device_list *out)
{
    ...
    sqlite3_stmt *dev_stmt = NULL;
    sqlite3_stmt *online_stmt = NULL;
    sqlite3_stmt *cap_stmt = NULL;
    int count = 0;
    int i;
    int rc;
    ...
    rc = sqlite3_open_v2(wemo_device_db, &dev_db, SQLITE_OPEN_READONLY, NULL);
    if (rc != SQLITE_OK) {
        if (dev_db != NULL) {
    ...
    sqlite3_bind_int(dev_stmt, 1, WE_DEVICE_LIST_MAX_ITEMS);
    ...
    rc = sqlite3_open_v2(wemo_state_db, &state_db, SQLITE_OPEN_READONLY, NULL);
    if (rc != SQLITE_OK) {
        if (state_db != NULL) {
    ...
    sqlite3_busy_timeout(state_db, 2000);
    rc = sqlite3_prepare_v2(state_db,
        "SELECT is_online FROM state WHERE wemo_id = ?;",
        -1, &online_stmt, NULL);
    if (rc != SQLITE_OK) {
        sqlite3_close(state_db);
        out->count = count;
    ...
    rc = sqlite3_prepare_v2(state_db,
        "SELECT value FROM state_capability WHERE wemo_id = ? AND cap = ?;",
        -1, &cap_stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    for (i = 0; i < count; i++) {
        ...
        sqlite3_bind_int(online_stmt, 1, d->wemo_id);
        if (sqlite3_step(online_stmt) == SQLITE_ROW) {
            d->is_online = (sqlite3_column_int(online_stmt, 0) != 0) ? 1 : 0;
        }

        sqlite3_reset(cap_stmt);
        sqlite3_clear_bindings(cap_stmt);
        sqlite3_bind_int(cap_stmt, 1, d->wemo_id);
        sqlite3_bind_int(cap_stmt, 2, CAP_BINARY);
        if (sqlite3_step(cap_stmt) == SQLITE_ROW) {
            d->state = sqlite3_column_int(cap_stmt, 0) ? 1 : 0;
        }

        sqlite3_reset(cap_stmt);
        sqlite3_clear_bindings(cap_stmt);
        sqlite3_bind_int(cap_stmt, 1, d->wemo_id);
        sqlite3_bind_int(cap_stmt, 2, CAP_LEVEL);
        if (sqlite3_step(cap_stmt) == SQLITE_ROW) {
            lv = sqlite3_column_int(cap_stmt, 0);
            if (lv < 0) lv = 0;
            if (lv > 100) lv = 100;
            d->level = lv;
        }
...
static void broker_send_completion_event(const struct we_broker_completion *completion)
{
    ...
    if (route_sockfd >= 0) {
        if (wemo_ipc_send_to_sock(route_sockfd, &ipchdr, payload) != 0) {
            /*
             * The requesting client may disconnect before async completion arrives.
             * Treat that as a stale route and drop quietly instead of error spam.
             */
            proto_route_purge_sockfd(route_sockfd);
            LOG_DEBUG_MSG("completion dropped request_id=%u wemo_id=%d cmd=%d sockfd=%d (stale client route)",
...
static int wemo_ipc_process_message(int sockfd, const struct we_ipc_hdr *ipchdr, char *ipc_data)
{
    ...
    if (ipchdr->cmd == CMD_PROTO) {
        ...
        struct we_device_list dev_list;
        ...
        if (proto->magic != WE_PROTO_MAGIC || proto->version != WE_PROTO_VERSION) {
            ...
            } else if (proto->op == CMD_LIST_DEVICES) {
                memset(&dev_list, 0, sizeof(dev_list));
                status = wemo_ctrl_list_devices(&dev_list);
                if (status == WE_STATUS_OK) {
                    rsp_payload_len = sizeof(dev_list);
                    memcpy(rsp_data + sizeof(struct we_proto_hdr), &dev_list, sizeof(dev_list));
                    rsp_size += (int)rsp_payload_len;
                }
            } else {
                if (proto->op == CMD_SET &&
