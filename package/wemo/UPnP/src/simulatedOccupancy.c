...
int adjustFirstTimer(int startTimer, int endTimer)
{
    ...
    } else if(gpSimulatedDevice->selfIndex > 0) {  /* later devices */
...
void unsetSimulatedData(void)
{
    ...
    if(gpSimulatedDevice) {
        if(gpSimulatedDevice->pDevInfo) {	//free simulated device list
            ...
            /* global list of deviceIndex and UDN is already freed in
               unsetSimulatedData. So making it NULL here. */
            g_devList = NULL;
        }
...
int setSimulatedRuleFile(void *deviceList)
{
    ...
    if(deviceListXml) {
        ...
        if (!fps) {
            ...
            ret = PLUGIN_FAILURE;
            if (localdeviceListXml) {
                free(localdeviceListXml);
                localdeviceListXml=NULL;
            }
            return ret;
        }
...
int setSimDevicesBinaryStateCount(int *devOff, int *devOn)
{
    ...
    *devOn = 0;
    ...
    APP_LOG("Simulated",LOG_DEBUG, "DevNumOn: %d, DevNumOff: %d",*devOn, *devOff);
...
int updateDiscoveredDevicesListInfo(void)
{
    ...
    int simdevicecnt = -1, i = 0, index = 0;
...
void enqueInAppNotification(int ruleId, char *ruleMsg, int ruleFreq)
{
    ...
    int ruleMsgLen = strlen(ruleMsg) + 1;
    ...
    /*
    notifyRuleID is used at cloud to distinguish different events in a home that need to be
    separated by ruleFreq seconds.  Away mode rule currently uses homeid as notifyRuleId.
    For any new APNS/GCM notification notifyRuleID must be altered.
    */

    qNode->ruleMSG = (char*)CALLOC(1, ruleMsgLen);
    strncpy(qNode->ruleMSG, ruleMsg, ruleMsgLen-1);
    qNode->ruleFreq = ruleFreq;
...
void sendInAppNotification(int ruleId, char *ruleMsg, int ruleFreq)
{
    char* parameter[] = {"RuleMessage"};
    char *value[1];

    value[0] = (char *)MALLOC(SIZE_512B);

    if (value[0] == NULL) {
        APP_LOG("DEVICE:rule", LOG_ERR, "MALLOC failed!!");
        return;
    }
    snprintf(value[0], SIZE_512B, "%d|%d|%lu|%s", ruleId, ruleFreq, GetUTCTime(), ruleMsg);

    APP_LOG("DEVICE:rule", LOG_DEBUG, "UpnpNotify %s value:(ruleId|ruleFreq|ruleExecutionTS|ruleMsg) %s",
            parameter[0], value[0]);
...
void sendManualToggleNotification(void)
{
    ...
    if(!ruleId) {
        /* if this is true, it is the source device
           for LP away and the ruleId is stored in gLongPressRuleActive
           after parsing the ruleDB. */
        if(g_longPressOccurred) {
            ruleId = gLongPressRuleActive;
        }
        /* otherwise if this is true, it is one of the target devices
           for LP away and the ruleId is received in call
           to SetAwayRuleTask */
        else if(g_longPressAwayRunning) {
            ruleId = g_LongPressAwayRuleID;
        }
    }

    if(ruleId) {
        APP_LOG("DEVICE:rule", LOG_DEBUG, "Rule ID %d is Active, Enqueue manual toggle notification!", ruleId);
        //        enqueInAppNotification(ruleId, "RULECONFIG1", 300);
        /* Since remote access will be taken cared by wemo_remote, send UpnpNotify */
        sendInAppNotification(ruleId, "RULECONFIG1", 300);
    }
}

/************************************************************************
 * Function: discoverDevices
 *    Discovers the devices and updates the list of discovered
 *    devices.
 *  Parameters:
 *    N.A.
 *  Return:
...
void notifyManualToggle(void)
{
    ...
    /* push notification and manual toggle date are not needed
       in case of long press away rule. */
    ...
    /* discover devices before sending notifyManualToggle. */
    discoverDevices();
...
int evaluateNextActions(int startTimer, int endTimer)
{
    ...
    int upnpRespWait = SIMULATED_UPNP_RESP_WAIT;
     ...
     */
    if(gpSimulatedDevice && (g_longPressAwayRunning || !strlen(gpSimulatedDevice->manualTriggerDate))) {

        /* discover devices before sending the action. */
        discoverDevices();
        ...
        if(curState) {
            ...
            APP_LOG("Rule", LOG_DEBUG, "deviceOnForTS: %d, curTime: %d DevSelfOnTS: %d", deviceOnForTS, (int)curTime, gpSimulatedDevice->onTS);
        }
        duration = daySeconds();
        ...
        /*
           ...
           - Device woke up to take action but is OFF right not but already MAX or more allowed devices are ON, so don't toggle
...
void cleanupAwayRule(int ruleReload)
{
    ...
    unsetSimulatedData();
    if(!gProcessData && !gLongPressRuleActive )
        StopPluginCtrlPoint();
    if(!ruleReload) {
        /* clean any long press away rule related data */
        if(LONG_PRESS_AWAY_ACTIVE)
            cleanUpForLongPressAway();
