...
pthread_t numTestApp_thread;
pthread_t MaxThreadCheck_thread;
...
void WriteRebootStatus()
{
    ...
    //CPU Check
    {
        ...
        if( retVal ) {
            unsigned long long diffTotal,diffI;
...
//Detect the number of threads running
        ...
        if( MAX_NUM_THREADS < numThreads ) {
            ...
            pthread_create(&MaxThreadCheck_thread, &wemoApp_attr, MaxThreadMonitor , (void *)&procId);
        }
