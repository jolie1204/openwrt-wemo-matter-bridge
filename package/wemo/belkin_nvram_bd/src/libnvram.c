...
*
* Please See http://wiki.nikleb.com:8090/display/SWE/NVRAM+TNG for an
...
#else
#define START_DAEMON_CMD   "nvramd"
#define NVRAM_FILENAME     "/etc/nvram.bin"
#endif
...
static void ErrLog(const char *Function,int Line);
...
// Initialize index
    ...
    while((StrLen = strlen(cp)) != 0) {
        // Validate string: <name>=<value>
        // we must have a '=' character and a nonzero length <name> and <value>
        do {
            if((Value = strchr(cp,'=')) == NULL) {
                // No Equals, ignore the string
                LOG("%s: Removing garbage string '%s' from NVRAM",__FUNCTION__,cp);
            ...
            *Value = 0;
            if(strlen(cp) > 0 && strlen(&Value[1]) > 0) {
                // Valid <name>/<value> pair, add it to the index
                *Value = '=';
                p->Index[i++] = Offset;
    ...
    if(i >= MAX_TBL_ENTRIES) {
        ...
        // Make sure there is only one entry per variable
        for(i = 1; i < p->NumEntries; i++) {
            Name = &p->Nvram[p->Index[i-1]];
            Name1 = &p->Nvram[p->Index[i]];
            cp = strchr(Name,'=');
            cp1 = strchr(Name1,'=');
            StrLen = cp - Name;
            if(StrLen == (cp1 - Name1)) {
                if(strncmp(Name,Name1,StrLen) == 0) {
                    // Same variable, drop the duplicate
                    LOG("%s: removing duplicate variable '%s'",__FUNCTION__,Name);
                    p->NumEntries--;
                    if(p->Flags & FLAG_GEMTEK) {
                        *pCount -= 1;
                    }
                    if(i < p->NumEntries) {
                        Entries2Copy = p->NumEntries-i;
                        memmove(&p->Index[i],&p->Index[i+1],
                                Entries2Copy * sizeof(int));
                    }
                }
            }
        }
...
// Update SysLogLevel
    NvramGetInt(VAR_SYSLOGLEVEL,&p->SysLogLevel);
}
...
/* should free buffer returned by NvramToJson */
char *NvramToJson()
{
    const char *Variable;
    int Next = -1;
    ...
    for (;;) {
        ...
        if (Variable == NULL) {
            ...
            if (count) {
                buffer = realloc(buffer, strlen(buffer) + 1 + strlen(","));
                sprintf(buffer + strlen(buffer), ",");
            }
        }
        str = malloc(strlen(Variable) + 1);
        strcpy(str, Variable);
        token = strtok_r(str, "=", &saveptr);
        /* add 3 for two \" and one : */
        buffer = realloc(buffer, (strlen(buffer) + 1) + strlen(token) + 3);
        sprintf(buffer + strlen(buffer), "\"%s\":", token);

        token = str + strlen(token) + 1;
        /* add 2 for two \" */
        buffer = realloc(buffer, (strlen(buffer) + 1) + strlen(token) + 2);
        sprintf(buffer + strlen(buffer), "\"%s\"", token);
        count++;
        free(str);
    }
    buffer = realloc(buffer, strlen(buffer) + strlen(buffer) + strlen("}"));
    sprintf(buffer + strlen(buffer), "}");
...
int NvramDaemon(bool bForground)
{
    ...
    do {
        ...
        p->DaemonPID = getpid();
        LOG("NVRAM daemon starting - pid %d",p->DaemonPID);
        GetSemaphore(p);
        p->Flags &= ~FLAG_DAEMON_STARTING;
        p->DaemonStarts++;
        ...
        for( ; ; ) {
            // NB: the semaphore must be owned at this point.
            // Check the commit timers
            GetUpTime();
            if(p->CommitTimer != 0 && p->CommitTimer < gUpTime) {
                // The commit timer has expired, flush the cache to flash now
                p->CommitTimer = 0;
                DLOG("CommitTimer timeout, calling NvramCommitInternal");
                NvramCommitInternal();
            } else {
                if(p->CommitTimer != 0) {
                    // Sleep until commit timer expires
                    ...
                    if(p->SemOwnerTID == MyTID) {
                        ELOG("%s:#%d: Error - semaphore still owned, count: %d",
                             __FUNCTION__,__LINE__,p->SemCount);
                    }
                    DLOG("calling usleep for %d microseconds",WaitTime);
                    usleep(WaitTime);
                    GetSemaphore(p);
                } else {
                    // Nothing to do now, sleep until signaled
                    ReleaseSemaphore(p);
                    if(p->SemOwnerTID == MyTID) {
                        ELOG("%s:#%d: Error - semaphore still owned, count: %d",
                             __FUNCTION__,__LINE__,p->SemCount);
...
static void GetSemaphore(SharedMem *p)
{
    ...
    for( ; ; ) {
        ...
        if(clock_gettime(CLOCK_REALTIME,&TimeOut) != 0) {
            // NB: do NOT exit the wait loop on error return from clock_gettime.
        ...
        // Couldn't get semaphore, check if the owner is still alive.
        OwnerTID = p->SemOwnerTID;
        if(kill(OwnerTID,0) == -1) {
            if(errno == ESRCH) {
                // The owner MAY have died, double check that the owner didn't
                // released the semaphore while we were checking his status
                if(p->SemOwnerTID == OwnerTID) {
                    // Yup he's dead, release the semaphore
                    ELOG("NVRAM Semaphore owner tid %d died, SemCount: %d",
                         OwnerTID,p->SemCount);
                    p->SemOwnerTID = -1;
                    p->SemCount = 0;
                    // Make sure the semaphore is in the state we expect
                    if(sem_getvalue(&p->Semaphore,&Value) == -1) {
                        ELOG("sem_getvalue failed - %s",strerror(errno));
                    } else if(Value > 0) {
                        ELOG("Error invalid semaphore value: %d",Value);
                    } else if(sem_post(&p->Semaphore) == -1) {
                        ELOG("sem_post failed - %s",strerror(errno));
                    } else {
                        ELOG("Semaphore state reset");
                    }
                } else {
                    // Hope, just a nasty race
                    ELOG("Previous owner has released semaphore");
                }
            } else if(errno != EINTR) {
                // This shouldn't happen, log the error and keep trying
                ELOG("kill failed - %s",strerror(errno));
            }
        } else {
            // The sempaphore owner is still alive
            if(!bLoggedTimeout) {
        ...
        // Release the RecoverySemaphore and keep waiting
        for( ; ; ) {
...
int NvramStatus()
{
    ...
    if(p->Errors > 0) {
        ...
        printf("\n");
        printf("Errors: %d\n",p->Errors);
        for(i = 0; i < p->Errors && i < MAX_ERR_LOG_ENTRIES; i++) {
            printf("  %s#%d\n",p->Errs[i].Function,p->Errs[i].Line);
        }
    }
...
static int get_config(NvramInfo *p)
{
    ...
    do {
        ...
        while(fgets(Line,sizeof(Line),fp) != NULL) {
            ...
            Converted = sscanf(Line,"%15s %*x %x %x",p->MtdName,&p->NvramLen,
                               &p->EraseSize);
            if(Converted == 3) {
                Ret = 0;
                break;
...
static void ErrLog(const char *Function,int Line)
{
    ...
    if(p != NULL) {
        if(p->Errors < MAX_ERR_LOG_ENTRIES - 1) {
            p->Errs[p->Errors].Function = Function;
            p->Errs[p->Errors].Line = Line;
        }
        p->Errors++;
    }
}
...
void logger(int Level,const char *fmt,...)
{
    ...
    if(p != NULL && Level <= p->SysLogLevel) {
        // Log on console
        if(gConsoleFD < 0) {
            // Open console
            gConsoleFD = open("/dev/console",O_WRONLY | O_NONBLOCK);
        }

        if(gConsoleFD >= 0) {
            // Copy the log message to console
            write(gConsoleFD,Line,strlen(Line));
        }
    }
#else
    if(gLogfp != NULL) {
