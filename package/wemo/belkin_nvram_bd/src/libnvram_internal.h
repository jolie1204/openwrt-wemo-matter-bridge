...
#ifdef __MIPSEL__
#define SYSLOG(level,format,...) logger(level,"nvram/%d/%s: " format "\n", (int) syscall(SYS_gettid), __FUNCTION__, ## __VA_ARGS__)
#else
static FILE *gLogfp = NULL;
#define SYSLOG(level,format,...) logger(0,"%s: " format "\n", __FUNCTION__, ## __VA_ARGS__)
#endif
...
#define VAR_SYSLOGLEVEL    "SysLogLevel"
...
// Statistics
    ...
    int Errors;       //
    int NvramErrors;  // Physical NVRAM access errors
...
#define MAX_ERR_LOG_ENTRIES   10
    struct {
        const char *Function;
        int Line;
    } Errs[MAX_ERR_LOG_ENTRIES];
