 ...
 */
pthread_mutex_t g_ulog_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void ulog_set_output()
{
    ...
    //If homekit.console file is existed, the other console options are ignored...
    if( stat(ULOG_HOMEKIT_CONSOLE, &s) == 0 ) {
        return;
    }
...
{
    struct stat s;
    pthread_mutex_lock(&g_ulog_mutex);
    ...
    ulog_set_output();
    pthread_mutex_unlock(&g_ulog_mutex);
}
...
void ulog_debug_packet (UCOMP comp, USUBCOMP sub, const char *mesg, int length)
{
    ...
    if( mesg && length ) {
        while( length > 0 ) {
            ...
            syslog(LOG_DEBUG, "%s.%s %s ", getcomp(comp), getsubcomp(sub), szDebugMsg);
            pos += 128;
            length -= 128;
        }
    }
...
void ulog_debugf (UCOMP comp, USUBCOMP sub, const char *fmt, ...)
{
    ...
    snprintf(sfmt, sizeof(sfmt), " %s.%s %s\n",
...
void ulog_debug_console(const char* tag, const char *fmt, ...)
{
    ...
    time(&now);
    ts = localtime(&now);
    snprintf(format, sizeof(format), "%s %s", tag, fmt);
    va_start(list, fmt);
    vprintf(format, list);
    va_end(list);
    pthread_mutex_unlock(&g_ulog_mutex);
